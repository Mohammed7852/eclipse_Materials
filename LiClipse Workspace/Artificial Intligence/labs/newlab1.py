...
class Problem(object):
    ...
    def result(self, state, action):
       return Node.solution(state,action)
    ...
    def create_heuristic (self,current):
        i=0 
        counter=0
        while i<len(current):
            if current[i]!=self.goal[i]:
                counter+=1
            i+=1 
...
class Node:
    ...
    def expand_node(self, node, nodes ):
        expanded_nodes = []
        expanded_nodes.append( create_node( move_up(
        node.state ), node, "u", node.depth + 1, 0 ) )
        expanded_nodes.append( create_node( move_down(
        node.state ), node, "d", node.depth + 1, 0 ) )
        expanded_nodes.append( create_node( move_left(
        node.state ), node, "l", node.depth + 1, 0 ) )
        expanded_nodes.append( create_node( move_right(
        node.state), node, "r", node.depth + 1, 0 ) )
        # Filter the list and remove the nodes that are
         #expanded_nodes = [node for node in expanded_nodes
