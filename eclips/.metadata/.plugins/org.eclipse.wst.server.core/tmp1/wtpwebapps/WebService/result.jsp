...
<%
	...
	try {
		...
		WebTarget target = client.target(UriBuilder.fromUri("http://localhost:8080/WebService").build());
		...
		Object obj = parser.parse(target.path("rest").path("calculator").path(Integer.toString(value1))
				.path(Integer.toString(value2)).request().accept(MediaType.TEXT_PLAIN).get(String.class)
				.toString());
		JSONObject jsonObj = (JSONObject) obj;
		sum = Integer.parseInt(jsonObj.get("add").toString());
		sub = Integer.parseInt(jsonObj.get("sub").toString());
		mult = Integer.parseInt(jsonObj.get("mult").toString());
	} catch (Exception e) {
		e.printStackTrace();
	}
%>
